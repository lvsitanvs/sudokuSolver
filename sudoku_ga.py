...
numDigits: int = 9  # Number of digits (in the case of standard Sudoku puzzles, this is 9).


class Population(object):
    """ A set of candidate solutions to the Sudoku puzzle. These candidates are also known as the chromosomes in the
    population."""
    ...
    def seed(self, num_candidates, given):
        ...
        helper = Candidate()
        helper.values = [[[] for _ in range(0, numDigits)] for _ in range(0, numDigits)]
    ...
    def sort(self):
        ...
        self.candidates.sort(key=lambda x: x.fitness, reverse=True)  # Sort in descending order. Modified by Ven
        return
...
class Candidate(object):
    ...
    def update_fitness(self):
        """ The fitness of a candidate solution is determined by how close it is to being the actual solution to the
        puzzle. The actual solution (i.e. the 'fittest') is defined as a 9x9 grid of numbers in the range [1,
        9] where each row, column and 3x3 block contains the numbers [1, 9] without any duplicates; if there are any
        duplicates then the fitness will be lowed."""

        row_count = numpy.zeros(numDigits)
        column_count = numpy.zeros(numDigits)
        block_count = numpy.zeros(numDigits)
        row_sum = 0
        column_sum = 0
        block_sum = 0

        for i in range(0, numDigits):  # For each row...
            for j in range(0, numDigits):  # For each number within it...
                row_count[self.values[i][j] - 1] += 1  # ...Update list with occurrence of a particular number.

            row_sum += (1.0 / len(set(row_count))) / numDigits
            row_count = numpy.zeros(numDigits)
        ...
        for i in range(0, numDigits):  # For each column...
            ...
            column_sum += (1.0 / len(set(column_count))) / numDigits
            column_count = numpy.zeros(numDigits)
        ...
        for i in range(0, numDigits, 3):
            for j in range(0, numDigits, 3):
                ...
                block_sum += (1.0 / len(set(block_count))) / numDigits
                block_count = numpy.zeros(numDigits)
...
class Tournament(object):
    """ The crossover function requires two parents to be selected from the population pool. The Tournament class is
    used to do this.
    
    Two individuals are selected from the population pool and a random number in [0, 1] is chosen. If this number is
    less than the 'selection rate' (e.g. 0.85), then the fitter individual is selected; otherwise, the weaker one is
    selected.
    """
...
class CycleCrossover(object):
    ...
    creating a fitter child candidate. Cycle crossover is used here (see e.g. A. E. Eiben, J. E. Smith. Introduction
    to Evolutionary Computing. Springer, 2007)."""
    ...
    def crossover_rows(self, row1, row2):
        child_row1 = numpy.zeros(numDigits)
...
class Sudoku(object):
    ...
    def solve(self):
        ...
        for generation in range(0, num_generations):
            ...
            best_fitness = 0.0
            for c in range(0, num_candidates):
            ...
            for count in range(num_elites, num_candidates, 2):
                ...
                # Mutate child1.
                child1.update_fitness()  # Update fitness before mutation. Add by Ven
                ...
                # Mutate child2.
                child2.update_fitness()  # Update fitness before mutation. Add by Ven
